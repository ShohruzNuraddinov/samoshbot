...
from tgbot.handlers.order.keyboards import make_category_btn, make_products_btn, make_cart_quantity_btn, cart_items_keyboard
...
def categories(update: Update, context: CallbackContext):
    ...
    if not categories:
        update.message.reply_text(
            "Kategoriya topilmadi", reply_markup=main_keyboard()
        )
        return ConversationHandler.END

    update.message.reply_text(
        "Categories", reply_markup=make_category_btn(cart))
...
def product_detail(update: Update, context: CallbackContext):
    ...
    if update.message.text == "🔙 Orqaga":
        update.message.reply_text(
            "Kategoriya", reply_markup=make_category_btn(cart))
        return states.CATEGORY
    ...
    if not product:
        update.message.reply_text(
            "Mahsulot topilmadi", reply_markup=main_keyboard()
        )
        return states.PRODUCTS
...
def cart_save(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        "Mahsulotni kerakli miqdorini kiriting", reply_markup=main_keyboard())
    return ConversationHandler.END
...
def cart_update(update: Update, context: CallbackContext):
    ...
    update.callback_query.message.edit_text(
        text, reply_markup=btn, parse_mode='HTML')
